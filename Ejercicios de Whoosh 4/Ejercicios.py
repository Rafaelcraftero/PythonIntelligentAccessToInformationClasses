...
def almacenar_datos():
    ...
    schem = Schema(titulo=TEXT(stored=True), precio=NUMERIC(stored=True, numtype=float),
                   tematicas=KEYWORD(stored=True, commas=True, lowercase=True), complejidad=ID(stored=True),
    ...
    for j in lista:
        ...
        writer.add_document(titulo=str(j[0]), precio=float(str(j[1])), tematicas=str(j[2]), complejidad=str(j[3]),
                            jugadores=str(j[4]), detalles=str(j[5]))
        i += 1
    writer.commit()
