...
def extraer_pelicula(url):
    ...
    for link_pelicula in lista_link_peliculas:
        ...
        if (datos.find("dt", string="PaÃ­s")):
            ...
            pais = "Desconocido"
        fecha = datetime.strptime(datos.find("dt", string="Estreno en EspaÃ±a").find_next_sibling("dd").string.strip(),
                                  '%d/%m/%Y')
...
def almacenar_datos():
    ...
    for pelicula in lista:
        ...
        writer.add_document(titulo=str(pelicula[0]), titulo_original=str(pelicula[1]), pais=str(pelicula[2]),
                            fecha=pelicula[3], director=str(pelicula[4]), generos=str(pelicula[5]),
                            sinopsis=str(pelicula[6]), url=str(pelicula[7]))
        i += 1
    writer.commit()
        ...
        with ix.searcher() as searcher:
            ...
            if entrada not in lista_generos:
                messagebox.showinfo("Error",
                                    "El criterio de bÃºsqueda no es un gÃ©nero existente\nLos gÃ©neros existentes son: " + ",".join(
                                        lista_generos))
                return
...
def modificar_fecha():
    def modificar():
        ...
        respuesta = messagebox.askyesno(title="Confirmar",
                                        message="Esta seguro que quiere modificar las fechas de estrenos de estas peliculas?")
        if respuesta:
            writer = ix.writer()
            for r in lista:
                writer.update_document(url=r['url'], fecha=datetime.strptime(str(en1.get()), '%Y%m%d'),
                                       titulo=r['titulo'], titulo_original=r['titulo_original'], pais=r['pais'],
                                       director=r['director'], generos=r['generos'], sinopsis=r['sinopsis'])
            writer.commit()
