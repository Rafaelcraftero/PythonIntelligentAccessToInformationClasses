...
def almacenar_bd(libros):
    ...
    for i in libros:
        if i[3] == 'Unknown':
            i[3] = 0
        conn.execute("""INSERT INTO BOOKS (ISBN, TITLE, AUTHOR, YEAR, PUBLISHER) VALUES (?,?,?,?,?)""",
                     (i[0], i[1], i[2], int(i[3]), i[4]))
    conn.commit()
...
def listar(cursor):
    ...
    for row in cursor:
        ...
        lb.insert(END, s)
        lb.insert(END, "------------------------------------------------------------------------")
        s = "     ISBN: " + row[0] + ' | AUTOR: ' + row[2] + ' | AÑO: ' + (
            str(row[3]) if row[3] != 0 else "Desconocido")
        lb.insert(END, s)
        lb.insert(END, "\n\n")
