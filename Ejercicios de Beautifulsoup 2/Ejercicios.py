...
def almacenar_bd():
    ...
    for link_pelicula in lista_link_peliculas:
        ...
        conn.execute(
            """INSERT INTO PELICULA (TITULO, TITULO_ORIGINAL, PAIS, FECHA, DIRECTOR, GENEROS) VALUES (?,?,?,?,?,?)""",
...
def buscar_por_titulo():
    def listar(event):
        ...
        conn.text_factory = str
        cursor = conn.execute(
            "SELECT TITULO, PAIS, DIRECTOR FROM PELICULA WHERE TITULO LIKE '%" + str(entry.get()) + "%'")
        conn.close
