...
def buscar_por_tematicas():
    def listar(event):
        ...
        conn.text_factory = str
        cursor = conn.execute(
            "SELECT TITULO, TEMATICAS, COMPLEJIDAD FROM JUEGOS where TEMATICAS LIKE '%" + str(tematica.get()) + "%'")
        conn.close
...
def buscar_por_complejidad():
    def listar(event):
        conn = sqlite3.connect('juegos.db')
        conn.text_factory = str
        cursor = conn.execute("SELECT TITULO, TEMATICAS, COMPLEJIDAD FROM JUEGOS where COMPLEJIDAD LIKE '%" + str(
            complejidad.get()) + "%'")
        conn.close
